...
VideoName <- readline(as.character("Coordinate file name (csv file): "))
CornerCoords <- readline(as.character("Corner coorcidates file name (txt file): "))

start_hour <- readline(as.character("Start hour: "))
start_hour <- as.numeric(start_hour)
start_min <- readline(as.character("Start min: "))
start_min <- as.numeric(start_min)
ProcessingFps <- readline(as.character("Freq of pulling coordinates (fps probably 3): "))
ProcessingFps <- as.numeric(ProcessingFps)
threshold <-  readline(as.character("Threshold for tracking bee (.98 is best): "))
...
CorCoord <- read.csv(paste(CornerCoords, ".txt", sep = ""), header = T, row.names = NULL, sep="\t")
...
while (i <= beeNum){
  ...
  png(file=paste(VideoName, "_ind", i, "_pathPlots.png",sep=""))
...
for (jp in (1:((ncol(df_bee))/2))){
  ...
  write_xlsx(plot_table,paste(VideoName,"_SpeedData.xlsx")) 
